...
world.beforeEvents.playerLeave.subscribe((eventData) => {
    ...
    if (leavingPlayer.hasTag('partyleader')) {
        ...
        if (remainingPlayers.length > 0) {
            ...
            remainingPlayers.forEach(player => {
                player.runCommandAsync(
                    `tellraw @s {"rawtext":[{"text":"§e${leavingPlayer.nameTag} §cleft the party. §e${newLeader.nameTag} §ais now the party owner."}]}`
        ...
        remainingPlayers.forEach(player => {
            player.runCommandAsync(
                `tellraw @s {"rawtext":[{"text":"§e${leavingPlayer.nameTag} §cleft the party."}]}`
