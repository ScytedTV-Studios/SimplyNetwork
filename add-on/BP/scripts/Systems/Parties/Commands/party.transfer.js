    ...
    if (message.startsWith("!party transfer")) {
        ...
        sender.runCommandAsync('tag @s remove partyleader')
            ...
            .then(() => sender.runCommandAsync(
                `tellraw @s {"rawtext":[{"text":"§aParty ownership transferred to §e${newOwnerPlayer.name}§a."}]}`
            ))
            .then(() => newOwnerPlayer.runCommandAsync(
                `tellraw @s {"rawtext":[{"text":"§e${sender.name} §ahas transferred Party ownership to you."}]}`
            ))
            .catch((error) => {
