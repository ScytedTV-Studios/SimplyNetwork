...
world.events.blockBreak.subscribe((e)=>{
    ...
    const mine = Object.values(PrisonMine.data).find((mine)=>{
        return location.x >= mine.min.x && location.x <= mine.max.x && location.y >= mine.min.y && location.y <= mine.max.y && location.z >= mine.min.z && location.z <= mine.max.z;
    });
    if (!mine) return;
    mine.currentBlocks--;
    if (mine.currentBlocks <= mine.maxBlocks * mine.refillPercent / 100 && !mine.isRefilling) {
        mine.refill();
    }
});
