...
export class PrisonMine {
    ...
    refill() {
        ...
        world.getAllPlayers().forEach((p)=>{
            ...
            let loc = p.location;
            if (loc.x >= this.min.x && loc.x <= this.max.x && loc.y >= this.min.y && loc.y <= this.max.y && loc.z >= this.min.z && loc.z <= this.max.z) {
    ...
    editMineMenu(player) {
        const menu = new ActionFormData().title(`Edit Mine: ${this.name}`).body(`Location: ${this.min.x}, ${this.min.y}, ${this.min.z} to ${this.max.x}, ${this.max.y}, ${this.max.z}`).button("Refill Mine").button("Teleport to Mine").button("Edit Blocks").button("Mine Settings").button("Delete Mine").button("Go Back");
        //@ts-ignore
    ...
    editBlocksMenu(player) {
        ...
        this.blocks.elements.forEach((element)=>{
            let extra = "";
            if (element.element.permutation.key != "undefined" && element.element.permutation.value != "undefined") {
        ...
        menu = menu.body(currentBlocks).button("Add Block").button("Remove Block").button("Reset Blocks").button("Go Back");
        //@ts-ignore
        menu.show(player).then((result)=>{
            switch(result.selection){
                case 0:
    ...
    addBlockMenu(player) {
        const menu = new ModalFormData().title(`Add Block | Mine: ${this.name}`).textField("Block ID", "Block ID", "minecraft:stone").slider("Block Weight", 1, 100, 1, 50).textField("Permutation Key", "Permutation Key", "undefined").textField("Permutation Value", "Permutation Value", "undefined");
        //@ts-ignore
        menu.show(player).then((result)=>{
    ...
    settingsMenu(player) {
        let form = new ModalFormData().title(`Mine Settings | Mine: ${this.name}`).textField("Mine Refill Timer (in seconds)", "Set to -1 to disable", this.maxRefillTimer.toString()).slider("Auto-Refill Percentage | Percentage of mine left to auto-refill the mine (-1 to disable)", -1, 90, 1, this.refillPercent).textField("Teleport Coordinates", "X, Y, Z", `${this.teleportLoc.x}, ${this.teleportLoc.y}, ${this.teleportLoc.z}`);
        //@ts-ignore
        form.show(player).then((result)=>{
            ...
            const rawTeleportCoords = result.formValues[2].replace(" ", "").split(",");
            const teleportCoords = {
                x: parseInt(rawTeleportCoords[0]),
                y: parseInt(rawTeleportCoords[1]),
                z: parseInt(rawTeleportCoords[2])
            };
            if (isNaN(refillTimer) || isNaN(refillPercent) || isNaN(teleportCoords.x) || isNaN(teleportCoords.y) || isNaN(teleportCoords.z)) {
    ...
    constructor(name, pos1, pos2, blocks, isRefilling = false, currentBlocks = 0, maxRefillTimer = -1, refillPercent = 50, teleportLoc){
        ...
        this.maxBlocks = Math.abs(1 + this.min.x - this.max.x) * Math.abs(1 + this.min.y - this.max.y) * Math.abs(1 + this.min.z - this.max.z);
