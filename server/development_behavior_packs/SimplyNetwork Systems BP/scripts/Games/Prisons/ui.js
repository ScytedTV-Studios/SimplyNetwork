...
function createMine(player) {
    let form = new ModalFormData().title("Create New Mine").textField("Mine Name", "Mine Name").textField("Mine Coordinates (x1, y1, z1, x2, y2, z2)", "X1, Y1, Z1, X2, Y2, Z2");
    //@ts-ignore
    form.show(player).then((response)=>{
        if (!response.formValues || !response.formValues[0] || !response.formValues[1]) return;
        ...
        //calc total area
        const area = Math.abs(1 + min.x - max.x) * Math.abs(1 + min.y - max.y) * Math.abs(1 + min.z - max.z);
        if (area > 5000000) {
